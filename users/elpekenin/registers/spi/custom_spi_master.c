...
static pin_t slave_pins[SPI_COUNT];
...
SPIDriver* drivers[SPI_COUNT] = { SPI_DRIVERS };
...
__attribute__((weak)) void spi_init(uint8_t index) {
    static bool is_initialised[SPI_COUNT];
...
bool spi_start(pin_t slavePin, bool lsbFirst, uint8_t mode, uint16_t divisor, uint8_t index) {
    ...
    spiStart(&drivers[index], &spiConfig);
