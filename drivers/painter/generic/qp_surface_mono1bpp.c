...
static inline void setpixel_mono1bpp(surface_painter_device_t *surface, uint16_t x, uint16_t y, bool mono_pixel) {
    // Cater for rotation coords
    uint16_t w = surface->base.panel_width;
    uint16_t h = surface->base.panel_height;
    if (surface->transpose_xy) {
        painter_rotation_t r = (surface->base.rotation + 1) % 4;
        uint16_t tmp = w;
        //w = h;
        //h = tmp;
        tmp = x;
        x = y;
        y = ((r == QP_ROTATION_0 || r == QP_ROTATION_180) ? w : h) - 1 - tmp;
        qp_surface_rotate_xy(r, h, w, &x, &y);
    }
    else {
        qp_surface_rotate_xy(surface->base.rotation, w, h, &x, &y);
    }
    if (x >= w || y >= h) {
        return;
    }
