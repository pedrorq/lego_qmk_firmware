...
__attribute__((weak)) bool qp_sh1106_init(painter_device_t device, painter_rotation_t rotation) {
    ...
    // Init the internal surface -- this is where we actually deal with rotation
    if (!qp_init(&driver->oled.surface.base, rotation)) {
...
bool qp_sh1106_flush(painter_device_t device) {
    ...
    // TBD: Does viewport-like stuff on the physical display based on the surface's dirty region, then sends it.
    // For now we just draw the entire display.
    int stride   = driver->oled.base.panel_height / 8;
    int page_min = driver->oled.surface.dirty.t / 8;             // dirty top
    int page_max = ((driver->oled.surface.dirty.b + 7) / 8) - 1; // dirty bottom
page_min = 0;
page_max = stride - 1;
    for (int page = page_min; page <= page_max; ++page) {
        qp_comms_command(device, SH1106_PAGE_ADDR | page);
        int col_min = driver->oled.surface.dirty.l;     // dirty left
        int col_max = driver->oled.surface.dirty.r - 1; // dirty right
col_min = 0;
col_max = driver->oled.base.panel_width - 1;
        qp_comms_command(device, SH1106_SETCOLUMN_LSB | (col_min & 0x0F));
        qp_comms_command(device, SH1106_SETCOLUMN_MSB | ((col_min & 0xF0) >> 4));

        uint8_t idx = 0;
        uint8_t buf[128];
        for (int col = col_min; col <= col_max; ++col) {
            uint16_t p = page + stride * col;
            buf[idx]   = driver->framebuffer[p];
            ++idx;
            if (idx == sizeof(buf)) {
                qp_comms_send(device, buf, idx);
                idx = 0;
            }
        }
...
const struct oled_panel_painter_driver_vtable_t sh1106_driver_vtable = {
    .base =
        {
            .init            = qp_sh1106_init,
...
painter_device_t qp_sh1106_make_spi_device(pin_t chip_select_pin, pin_t dc_pin, pin_t reset_pin, uint16_t spi_divisor, int spi_mode) {
    ...
    for (uint32_t i = 0; i < SH1106_NUM_DEVICES; ++i) {
        ...
        if (!driver->oled.base.driver_vtable) {
            ...
            painter_device_t surface = qp_make_mono1bpp_surface_advanced(&driver->oled.surface, 1, panel_height, panel_width, driver->framebuffer);
            if (!surface) {
                return NULL;
            }

            // Surface coords are transposed
            driver->oled.surface.transpose_xy = true;

            // Setup the OLED device
            driver->oled.base.driver_vtable         = (const struct painter_driver_vtable_t *)&sh1106_driver_vtable;
            driver->oled.base.comms_vtable          = (const struct painter_comms_vtable_t *)&spi_comms_with_dc_vtable;
...
// Factory function for creating a handle to the SH1106 device
painter_device_t qp_sh1106_make_i2c_device(uint8_t i2c_address) {
    const uint16_t panel_width  = 128;
    const uint16_t panel_height = 64;
    for (uint32_t i = 0; i < SH1106_NUM_DEVICES; ++i) {
        sh1106_device_t *driver = &sh1106_drivers[i];
        if (!driver->oled.base.driver_vtable) {
            ...
            // Setup the OLED device
            driver->oled.base.driver_vtable         = (const struct painter_driver_vtable_t *)&sh1106_driver_vtable;
            driver->oled.base.comms_vtable          = (const struct painter_comms_vtable_t *)&i2c_comms_cmddata_vtable;
            driver->oled.base.native_bits_per_pixel = 1; // 1bpp mono
            driver->oled.base.panel_width           = panel_width;
            driver->oled.base.panel_height          = panel_height;
            driver->oled.base.rotation              = QP_ROTATION_0;
